...
namespace AdventOfCodeDay4Part2 {
    ...
    class SecurityThroughObscurity {
        ...
        public getSectorId(): number {
            for (let line of this._input) {
                let parsedLine: IparsedLineObject = this.parseLine(line);
                if (this.decodeLine(parsedLine) === "northpole object storage") {
                    return parsedLine.sectorId;
                }
            }
        }

        private decodeLine(line: IparsedLineObject): string {
            let decodedLine: string = "";

            for (let letter of line.name) {
                if (letter !== " ") {
                    letter = this.getDecodedLetter(letter, line.sectorId);
                }
                decodedLine += letter;
            }
            return decodedLine;
        ...
        private getDecodedLetter(letter: string, shiftTime: number): string {
            return String.fromCharCode(((letter.charCodeAt(0) - 97 + shiftTime) % 26) + 97);
        }

        private parseLine(line: string): IparsedLineObject {
            let matches: Array<string> = /(.*)\-(\d*)/.exec(line);

            return {
                "name": matches[1].replace(/-/g, " "),
                "sectorId": parseInt(matches[2], 10)
            };
        }
    }
    ...
    let testInputs: Array<ITestInputsObject> = [
        {
            "input": [
                ...
                "northpole-object-storage-0",
                "qzmt-zixmtkozy-ivhz-1"
            ],
            "result": 0
        }
    ];
    ...
    for (let testInput of testInputs) {
        let codeTest: SecurityThroughObscurity = new SecurityThroughObscurity(testInput.input);
        let test: number = codeTest.getSectorId();
    ...
    let solution: SecurityThroughObscurity = new SecurityThroughObscurity(input);
    console.log("Sector ID of the room where North Pole objects are stored: ", solution.getSectorId());
}
