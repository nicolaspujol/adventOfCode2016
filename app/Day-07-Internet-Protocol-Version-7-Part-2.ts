...
anywhere in the supernet sequences (outside any square bracketed sections), and a corresponding Byte Allocation Block,
...
namespace AdventOfCodeDay7Part2 {
    ...
    class InternetProtocolVersion7 {
        ...
        public getIPsSupportSSL(): number {
            let nb: number = 0;
        ...
        private isABA(str: string): boolean {
            ...
            for (let i: number = 1; i < str.length - 1; i++) {
                ...
                if (str[i] !== str[i + 1] && str[i + 1] === str[i - 1]) {
                    ...
                    if (hypernet ? aba[str[i + 1] + str[i]] : bab[str[i + 1] + str[i]]) {
                        return true;
                    }

                    if (hypernet) {
                        bab[str[i] + str[i + 1]] = true;
                    } else {
                        aba[str[i] + str[i + 1]] = true;
                    }
                }
            }
    ...
    for (let testInput of testInputs) {
        ...
        let test: number = codeTest.getIPsSupportSSL();
    ...
    console.log("IPs support SSL: ", solution.getIPsSupportSSL());
}
