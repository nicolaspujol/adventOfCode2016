...
namespace AdventOfCodeDay10Part1 {
    // Interfaces
    interface IMicroshipsResponsibleObject {
        "microchips": Array<number>;
    }
    interface IMicroshipsObject {
        "bot": Array<IMicroshipsResponsibleObject>;
        "output": Array<IMicroshipsResponsibleObject>;
    }

    class BalanceBots {

        private _input: Array<string>;
        private _compare: Array<number>;
        private _microchips: IMicroshipsObject;
        private _bot: string;

        public constructor(input: Array<string>, compare: Array<number>) {
            this._input = input;
        ...
        public getNumberOfTheBot(): string {
            this.initMicrochips();
            this.moveMicrochips();
            return this._bot;
        }

        private initMicrochips(): void {
            for (let instruction of this._input) {
                let matches: Array<string> = /value (\d+) goes to bot (\d+)/.exec(instruction);

                if (matches) {
                    this.defineMicroships("bot", matches[2]);
                    this._microchips.bot[matches[2]].microchips.push(parseInt(matches[1], 10));
                    this._input.splice(this._input.indexOf(instruction), 1);
                }
            }
        }

        private defineMicroships(type: string, microchips: string): void {
            if (!this._microchips[type][microchips]) {
                this._microchips[type][microchips] = {
                    "microchips": []
                };
            }
        }

        private isBotFound(botNumber: string): boolean {
            let lastPos: number = this._microchips.bot[botNumber].microchips.length - 1;
            return parseInt(this._microchips.bot[botNumber].microchips[lastPos], 10) === this._compare[0] &&
                parseInt(this._microchips.bot[botNumber].microchips[0], 10) === this._compare[1];
        }

        private moveMicrochips(): void {
            while (this._input.length > 0 && this._bot === "") {
                for (let instruction of this._input) {
                    let matches: Array<string> =
                        /^bot (\d+) gives low to (output|bot) (\d+) and high to (output|bot) (\d+)$/.exec(instruction);

                    if (matches) {
                        this.defineMicroships(matches[2], matches[3]);
                        this.defineMicroships("bot", matches[1]);
                        this.defineMicroships(matches[4], matches[5]);

                        if (this._microchips.bot[matches[1]].microchips.length === 2) {
                            this._microchips.bot[matches[1]].microchips.sort((a: number, b: number) => {
                                return a - b;
                            });

                            if (this.isBotFound(matches[1])) {
                                this._bot = matches[1];
                                return;
                            }

                            this._microchips[matches[2]][matches[3]].microchips
                                .push(this._microchips.bot[matches[1]].microchips[0]);
                            this._microchips.bot[matches[1]].microchips.splice(0, 1);

                            this._microchips[matches[4]][matches[5]].microchips
                                .push(this._microchips.bot[matches[1]].microchips[
                                    this._microchips.bot[matches[1]].microchips.length - 1
                                ]);
                            this._microchips.bot[matches[1]].microchips
                                .splice(this._microchips.bot[matches[1]].microchips.length - 1, 1);
    ...
    for (let testInput of testInputs) {
        let codeTest: BalanceBots = new BalanceBots(testInput.input, [5, 2]);
        let test: string = codeTest.getNumberOfTheBot();
    ...
    let solution: BalanceBots = new BalanceBots(input, [61, 17]);
    console.log("Number of the bot: ", solution.getNumberOfTheBot());
}
