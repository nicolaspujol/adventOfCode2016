...
You need to overwrite some disks with random-looking data to cover your tracks and update the local security
...
Combining all of these steps together, suppose you want to fill a disk of length 20 using an initial state of 10000:
    ...
    - Next, we start calculating the checksum;
        after one round, we have 0111110101, which 10 characters long (even), so we continue.
...
namespace AdventOfCodeDay16Part1 {

    class DragonChecksum {

        private _input: string;
        private _diskLength: number;

        public constructor(input: string, diskLength: number) {
            this._input = input;
            this._diskLength = diskLength;
        }
    ...
    for (let testInput of testInputs) {
        let codeTest: DragonChecksum = new DragonChecksum(testInput.input, 12);
        let test: string = codeTest.getChecksum();
     ...
     * SOLUTION
     */
    let input: string = "10111011111001111";

    let solution: DragonChecksum = new DragonChecksum(input, 272);
    console.log("First time you can press the button: ", solution.getChecksum());
}
