...
namespace AdventOfCodeDay10Part1 {
    ...
    class BalanceBots {
        ...
        private _bots: Array<number>;
        private _rules: Array<number>;
        private _shouldGiveMicroships: Array<string>;
        ...
        public constructor(input: Array<string>, compare: Array<number>) {
            ...
            this._bots = [];
            this._rules = [];
            this._shouldGiveMicroships = [];
        }

        public getProductOfOutput(): number {
            this.parseInput();
            this.giveMicrochips();

            return this._outputs[0][0] * this._outputs[1][0] * this._outputs[2][0];
        }

        private parseInput(): void {
            for (let instruction of this._input) {
                let match: Array<string> = /^value (\d+) goes to bot (\d+)$/.exec(instruction);
                if (match) {
                    if (!this._bots[match[2]]) {
                        this._bots[match[2]] = [];
                    }
                    this._bots[match[2]].push(parseInt(match[1], 10));
                    if (this._bots[match[2]].length === 2) {
                        this._shouldGiveMicroships.push(match[2]);
                    }
                    continue;
                }

                match = /^bot (\d+) gives low to (output|bot) (\d+) and high to (output|bot) (\d+)$/.exec(instruction);
                if (match) {
                    this._rules[match[1]] = {
                        "highNumber": match[5],
                        "highType": match[4],
        ...
        private giveMicrochips(): void {
            while (this._shouldGiveMicroships.length > 0) {
                let botID: string = this._shouldGiveMicroships[this._shouldGiveMicroships.length - 1];

                this._bots[botID].sort((a: number, b: number) => a - b);
                this._shouldGiveMicroships.pop();
                this.moveMicrochips(
                    this._rules[botID].highType,
                    this._rules[botID].highNumber,
                    this._bots[botID][1]
                );
                this.moveMicrochips(
                    this._rules[botID].lowType,
                    this._rules[botID].lowNumber,
                    this._bots[botID][0]
                );
                this._bots[botID] = undefined;
            }
        }

        private moveMicrochips(type: string, chipNumber: string, currentChip: number): void {
            let destinations: Array<Array<number>> = this["_" + type + "s"];
            let destination: Array<number> = destinations[chipNumber] || (destinations[chipNumber] = []);
            destination.push(currentChip);
            if (type === "bot" && destination.length === 2) {
                this._shouldGiveMicroships.push(chipNumber);
            }
        }
    }
    ...
    for (let testInput of testInputs) {
        ...
        let test: number = codeTest.getProductOfOutput();
    ...
    console.log("Product of 3 outputs: ", solution.getProductOfOutput());
}
