...
namespace AdventOfCodeDay4Part1 {
    ...
    class SecurityThroughObscurity {
        ...
        public getSum(): number {
            ...
            for (let line of this._input) {
                ...
                sum += this.isRealRoom(parsedLine);
            }
            return sum;
        }

        private isRealRoom(line: IparsedLineObject): number {
            if (line.checkSum === this.getLetterGroups(line.name)) {
                return line.sectorId;
            }
            return 0;
        }

        private getLetterGroups(lineName: string): string {
            let checkSum: Array<string> = lineName.split("");

            return this.sortByFrequencyAndRemoveDuplicates(checkSum).join("").substr(0, 5);
        }

        private parseLine(line: string): IparsedLineObject {
            let matches: Array<string> = /(.*)\-(\d*)\[(\w*)\]/.exec(line);
        ...
        private sortByFrequencyAndRemoveDuplicates(array: Array<string>): Array<string> {
            ...
            return uniques.sort((a: string, b: string): number => {
    ...
    for (let testInput of testInputs) {
        ...
        let test: number = codeTest.getSum();
    ...
    console.log("Sum of the sector IDs of the real rooms: ", solution.getSum());
}
